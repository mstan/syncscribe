...
class SubtitleWriter {
  ...
  /**
   * Generate SRT format string from segments (no file I/O)
   * @param {Array} segments - Transcription segments
   * @returns {string} SRT formatted string
   */
  generateSRTString(segments) {
    return this._generateSRT(segments);
  }

  /**
   * Generate WebVTT format string from segments (no file I/O)
   * @param {Array} segments - Transcription segments
   * @returns {string} WebVTT formatted string
   */
  generateVTTString(segments) {
    return this._generateWebVTT(segments);
  }
