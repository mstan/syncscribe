...
class ExpressApi {
  ...
  _setupRoutes() {
    ...
    this.app.post('/api/jobs/:id/upload', auth, express.raw({ type: 'audio/*', limit: '100mb' }), (req, res, next) => this._handleUploadAudio(req, res, next));
  ...
  async _handleCreateJob(req, res, next) {
    try {
      ...
      const audioKey = this.handler.r2.audioKey(audio_sha256);
      const uploadUrl = await this.handler.r2.getPresignedUploadUrl(audioKey);
   ...
   * Upload audio data for a job. Proxies the upload to R2.
   * Body: raw audio/flac data.
   */
  async _handleUploadAudio(req, res, next) {
    try {
      ...
      // Upload to R2
      const audioKey = this.handler.r2.audioKey(job.audio_sha256);
      await this.handler.r2.uploadBuffer(audioKey, req.body, 'audio/flac');

      debug('Audio uploaded for job %s: key=%s size=%d', jobId, audioKey, req.body.length);
      res.json({ ok: true });
    } catch (error) {
      next(error);
    }
  ...
  async _handleGetSubtitleUrl(req, res, next) {
    try {
      ...
      const downloadUrl = await this.handler.r2.getPresignedDownloadUrl(subtitle.object_key);
